...
{
    public partial class TemporaryStationaryPage : ContentPage
    {
        ...
        public TemporaryStationaryPage(byte[] bytes)
        {
            InitializeComponent();
        ...
        private void SetData(byte[] bytes)
        {
            ...
            ReferenceFrequency.Text = (frequency == 0) ? "No Reference Frequency" : GetFrequency(frequency);
        ...
        private async void ReadyStationaryScan_Clicked(object sender, EventArgs e)
        {
            if (ExistChanges())
            {
                if (IsDataCorrect())
                {
                    bool result = await SetTemporaryStationary();
                    if (result)
                        await Navigation.PushModalAsync(new StationaryScanningPage(
                            FrequencyTableNumber.Text, ScanTime.Text, ScanTimeout.Text, NumberOfAntennas.Text, StoreRate.Text, StationaryBytes), false);
                }
            }
            else
            {
        ...
        private async Task<bool> SetTemporaryStationary()
        {
            try
            {
                ...
                if (service != null)
                {
                    ...
                    if (characteristic != null)
                    {
                        ...
                        byte[] b = new byte[] { 0x7C, (byte)antennasNumber, (byte)externalData, (byte)scanTime, (byte)scanTimeout,
