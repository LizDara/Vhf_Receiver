...
{
    public partial class TemporaryMobilePage : ContentPage
    {
        ...
        public TemporaryMobilePage(byte[] bytes)
        {
            InitializeComponent();
        ...
        private async void ReadyMobileScan_Clicked(object sender, EventArgs e)
        {
            if (ExistChanges())
            {
                bool result = await SetTemporaryMobile();
                if (result)
                {
                    string gps = GPS.IsToggled ? "ON" : "OFF";
                    string autoRecord = AutoRecord.IsToggled ? "ON" : "OFF";
                    await Navigation.PushModalAsync(new MobileScanningPage(ScanTime.Text, FrequencyTableNumber.Text, gps, autoRecord, MobileBytes), false);
                }
            }
            else
                await Navigation.PopModalAsync(false);
        ...
        private async Task<bool> SetTemporaryMobile()
        {
            try
            {
                ...
                if (service != null)
                {
                    ...
                    if (characteristic != null)
                    {
                        ...
                        byte[] b = new byte[] { 0x7D, (byte)frequencyTableNumber, (byte)info, (byte)(scanTime * 10), 0, 0, 0, 0 };
